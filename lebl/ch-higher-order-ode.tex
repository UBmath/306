...
where $p(x) = \nicefrac{B(x)}{A(x)}$, $q(x) = \nicefrac{C(x)}{A(x)}$, and
$f(x) = \nicefrac{F(x)}{A(x)}$.
The word \emph{linear\index{linear equation}} means, for example, that the equation contains no quadratic or higher powers 
of $y$, $y'$, and $y''$.
...
\end{equation*}
Although it is much easier to remember that we just need to try $y_2(x) =
...
%or \appendixref{linalg:appendix}.
