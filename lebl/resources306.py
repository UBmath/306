...
'''Utilities for MTH 306 in Spring 2020
	John Ringland with improvements by Jim Faran'''
...
def fieldplotlinear2019(A,xmin,xmax,ymin,ymax,color='b',aspect=None,nx=20,boostarrows=1.,arrowheads=True,alpha=1):
	def F(X): 
		return np.dot(A,X)
	fieldplot2019(F,xmin,xmax,ymin,ymax,color=color,aspect=aspect,nx=nx,boostarrows=boostarrows,arrowheads=arrowheads,alpha=alpha)
...
def fieldplot2019(F,xmin,xmax,ymin,ymax,color='b',aspect=None,nx=20,boostarrows=1.,arrowheads=True,alpha=1):
    ...
    yr = ymax-ymin
    asp = get_aspect()
    ny = nx*asp # changed 1/7/20 int(nx*yr/xr)
    if aspect!=None:
        plt.subplot(111,aspect=aspect)
    X,Y = np.meshgrid( np.linspace(xmin,xmax,nx), np.linspace(ymin,ymax,ny) )
    X = X.flatten()
...
def fieldplot(F,xmin,xmax,ymin,ymax,color='b',aspect=None,nx=20,boostarrows=1.,arrowheads=True,alpha=1):
    # improved version Jan 2020 by Jim Faran
    #    better choice of grid spacing
    #    better arrowheads
    def f(x,y): return F((x,y))[0]
    def g(x,y): return F((x,y))[1]
    # Doing it this way is unfortunately twice the work (2 calls to F for every evaluation)
...
def phaseportrait2019(F,ics=[], *args, **kwargs):  # F is a vector-valued function of a vector argument
	# version used in Fall 2019 UB Edition 1
	#print(kwargs)
...
def phaseportrait(F,ics=[], *args, **kwargs):  # F is a vector-valued function of a vector argument
	# January 2020 for UB Edition 2
	# replaced odeint, now deprecated, with solve_ivp
	# Jim Faran finds it deals gracefully with trajectories that blow up in finite time
	#print(kwargs)
	def FF(t,X): return F(X)  # odeint allows non-autonomous system
	for ict in ics:
		ic = ict[:2]
		if len(ict)==3:
			# initial time as 0 and 3rd argument as final time
		...
		for t1 in t1s:
			t = np.linspace(0,t1,500)
			Y = solve_ivp(FF,[0,t1],ic)
			plt.plot(Y.y[0],Y.y[1],*args, **kwargs)
